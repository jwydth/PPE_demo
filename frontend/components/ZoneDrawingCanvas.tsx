...
export function ZoneDrawingCanvas() {
  ...
  const [calibrationPolygon, setCalibrationPolygon] = useState<fabric.Polygon | null>(null);
  ...
  useEffect(() => {
    if (fabricCanvas) {
      fabricCanvas.setDimensions({ width: dimensions.width, height: dimensions.height });
      ...
      if (fabricCanvas) {
        data.forEach(zone => {
          ...
          if (shape.type === 'polygon') {
            fabricObj = new fabric.Polygon(shape.points, { ...shape, selectable: true });
          } else if (shape.type === 'path') {
            fabricObj = new fabric.Path(shape.path, { ...shape, selectable: true });
  ...
  const getZoneStyles = (type: ZoneType) => {
    switch (type) {
      case "RESTRICTED": return { fill: "rgba(239, 68, 68, 0.3)", stroke: "#ef4444" };
      case "WALKWAY": return { fill: "rgba(34, 197, 94, 0.3)", stroke: "#22c55e" };
      case "FORKLIFT_PATH": return { fill: "rgba(234, 179, 8, 0.3)", stroke: "#eab308" };
      default: return { fill: "rgba(161, 161, 170, 0.3)", stroke: "#a1a1aa" };
    }
  };
  ...
  useEffect(() => {
    ...
    const handleMouseDown = (opt: fabric.IEvent) => {
      ...
      if (points.length > 2 && Math.abs(newPoint.x - points[0].x) < 20 && Math.abs(newPoint.y - points[0].y) < 20) {
        completePath();
        return;
      }
      ...
      if (points.length === 0) {
        ...
        if (segmentType === "line") {
          ...
          setPoints(prev => [...prev, newPoint]);
          setPathSegments(prev => [...prev, `Q ${newPoint.x} ${newPoint.y} ${newPoint.x} ${newPoint.y}`]);
        }
      }
    ...
    const handleMouseMove = (opt: fabric.IEvent) => {
      ...
      if (isAdjustingCurve) {
        ...
        newSegments[newSegments.length - 1] = `Q ${pointer.x} ${pointer.y} ${endPoint.x} ${endPoint.y}`;
  ...
  }, [fabricCanvas, tool, points, pathSegments, tempPath, segmentType, isAdjustingCurve, zoneType, isMonitoring]);
    ...
    const zonesToSave: ZoneConfiguration[] = objects
      ...
      .map(obj => {
        ...
        if (obj instanceof fabric.Path) {
          ...
          const d = pathObj.path.map((segment: any) => segment.join(' ')).join(' ');
          ...
          flattened = (obj as fabric.Polygon).points?.map(p => {
            const transformedPoint = fabric.util.transformPoint(new fabric.Point(p.x, p.y), matrix);
            return { x: transformedPoint.x / canvasWidth, y: transformedPoint.y / canvasHeight };
          }) || [];
        } else if (obj instanceof fabric.Circle) {
          const radius = (obj as fabric.Circle).radius || 0;
          for (let i = 0; i < 64; i++) {
            const angle = (i / 64) * 2 * Math.PI;
            const localPoint = new fabric.Point(radius * Math.cos(angle), radius * Math.sin(angle));
            const transformedPoint = fabric.util.transformPoint(localPoint, matrix);
            flattened.push({ x: transformedPoint.x / canvasWidth, y: transformedPoint.y / canvasHeight });
          }
        }

        return {
          video_name: videoName,
          zone_name: (obj as any).zoneName || `Zone ${Math.floor(Math.random() * 1000)}`,
          zone_type: (obj as any).zoneType || zoneType,
          dwell_threshold_seconds: 0,
          is_active: true,
          ui_shape_data: JSON.stringify(obj.toObject(['zoneType', 'zoneName', 'zoneId'])),
  ...
  const startMonitoring = async () => {
    ...
    try {
      ...
      alert("Failed to analyze video: " + (err instanceof Error ? err.message : "Unknown error"));
    } finally {
    ...
    <div className="flex flex-col gap-4 items-center w-full max-w-6xl mx-auto">
      <div className="flex flex-wrap gap-4 items-center justify-between w-full bg-zinc-900 p-3 rounded-lg border border-zinc-800">
        <div className="flex gap-2">
          <ToolButton active={tool === "select"} onClick={() => setTool("select")} disabled={isMonitoring}>Select</ToolButton>
          <ToolButton active={tool === "draw"} onClick={() => setTool("draw")} disabled={isMonitoring}>Draw</ToolButton>
          <ToolButton active={tool === "calibrate"} onClick={() => setTool("calibrate")} disabled={isMonitoring}>Calibrate</ToolButton>
          <div className="w-px h-6 bg-zinc-800 mx-1" />
          <ToolButton active={segmentType === "line"} onClick={() => setSegmentType("line")} disabled={tool !== "draw" || isMonitoring}>Line</ToolButton>
          <ToolButton active={segmentType === "curve"} onClick={() => setSegmentType("curve")} disabled={tool !== "draw" || isMonitoring}>Curve</ToolButton>
          <div className="w-px h-6 bg-zinc-800 mx-1" />
          <ToolButton active={false} onClick={deleteSelected} disabled={isMonitoring} className="border-red-900/50 text-red-500 hover:bg-red-500/10">Delete</ToolButton>
        </div>

        <div className="flex gap-4 items-center">
          {!isMonitoring && (
            <div className="flex gap-2 items-center">
              <span className="text-[10px] font-mono text-zinc-500 uppercase">Zone Type:</span>
              <select 
                value={zoneType} 
          ...
          <div className="flex gap-2">
            ...
            <button 
              ...
              disabled={isAnalyzing || !bgImage || !bgImage.type.startsWith("video/")}
              className={`
              ...
              `}
            >
              {isAnalyzing ? "ANALYZING..." : isMonitoring ? "STOP MONITOR" : "START MONITORING"}
            </button>
          </div>
        </div>
      </div>
      ...
      {!bgImage ? (
        <div className="w-full max-w-xl py-20">
          <UploadZone onFileSelect={file => { setBgImage(file); setVideoName(file.name); }} />
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_260px] gap-4 w-full">
          <div className="flex flex-col gap-2">
            <div className="flex justify-between items-center px-1 h-6">
              <span className="text-[10px] font-mono text-zinc-500 uppercase truncate max-w-[200px]">
                {isMonitoring ? "LIVE MONITORING" : "CONFIGURATION"}: {videoName}
              </span>
              ...
              {!isMonitoring && (
                <button onClick={() => setBgImage(null)} className="text-[10px] font-mono text-zinc-500 hover:text-zinc-300 underline uppercase">Change Media</button>
              )}
            </div>
          ...
          <div className="bg-zinc-900/50 border border-zinc-800 rounded-lg p-3 flex flex-col gap-3">
            <p className="font-mono text-[10px] text-zinc-600 tracking-widest uppercase">Monitoring Log</p>
            {!isMonitoring ? (
              <div className="text-[10px] font-mono text-zinc-500 space-y-4">
                ...
                <p>4. Click START MONITORING to run analysis and preview incursions.</p>
              </div>
            ) : (
              <div className="flex-1 overflow-y-auto space-y-2">
                {currentViolations.length === 0 ? (
                  <p className="text-[10px] font-mono text-zinc-600 italic">Scanning for incursions...</p>
                ) : (
                  currentViolations.map((v, i) => (
                    <div key={i} className="bg-red-500/10 border border-red-500/30 rounded p-2 animate-pulse">
                      <p className="text-[10px] font-mono text-red-400 font-bold">BREACH DETECTED</p>
                      <p className="text-[9px] font-mono text-red-300/70 mt-1">
                        Track ID: {v.track_id}<br/>
                        Frame: {v.frame_index}
                      </p>
                    </div>
                  ))
...
function ToolButton({ active, onClick, children, className = "", disabled = false }: { active: boolean, onClick: () => void, children: React.ReactNode, className?: string, disabled?: boolean }) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
